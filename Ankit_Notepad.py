...
main_application.title('Ankpad text editor')
...
date_icon=tk.PhotoImage(file='icons2/date_time.png')
...
font_size['values']=tuple(range(6,51,2))
font_size.current(3)
...
speech_icon=tk.PhotoImage(file='icons2/t_s.png')
speech_btn=ttk.Button(tool_bar,image=speech_icon)
speech_btn.grid(row=0,column=6,padx=5)
...
speech_btn.configure(command=text_speech)
...
#Date Time
def date_func(event=None):
    global url
    current_time=str(datetime.datetime.now())
    opp="The Current Date and Time is: "+current_time + "\n"
    if url:
        content = text_editor.get(1.0, tk.END)
        # print(content)
        with open(url, 'w', encoding='utf-8') as fw:
        ...
        content2 = str(text_editor.get(1.0, tk.END))
        url = filedialog.asksaveasfile(mode = 'w', defaultextension='.txt', filetypes=(('Text File', '*.txt'), ('All files', '*.*')))
        url.write(opp+content2)
    with open(url,'r') as fr:
        text_editor.delete(1.0,tk.END)
        text_editor.insert(1.0,fr.read())

edit.add_command(label='Time/Date', image = date_icon, compound=tk.LEFT, accelerator='F5',command=date_func)
...
def hide_statusbar():
    ...
    if show_statusbar:
        ...
        status_bar.pack(side=tk.BOTTOM)
        show_statusbar = True 
view.add_checkbutton(label='Tool Bar',onvalue=True,offvalue=False, image = tool_bar_icon, compound=tk.LEFT,command=hide_toolbar)
view.add_checkbutton(label='Status Bar',onvalue=True,offvalue=False, image = status_bar_icon, compound=tk.LEFT,command=hide_statusbar)
